...
class KinovaSimWebXRNode(Node):
    def __init__(self):
        ...
        # Create immobile base cost for fixing the base
        immobile_base_cost = np.zeros((self.model.nv,))
        immobile_base_cost[:3] = 100  # High cost for base motion (x, y, theta)
        ...
        # Include damping task to keep base fixed
        self.tasks = [self.end_effector_task, self.posture_task, self.damping_task]
    ...
    def sim_loop(self):
        with mujoco.viewer.launch_passive(
            ...
            while rclpy.ok() and viewer.is_running():
                ...
                # TODO: Implement gripper control when a gripper is available
                # The Kinova doesn't have a gripper in this example, but we store the command
                # for future implementation
                if hasattr(self, 'gripper_open'):
                    self.get_logger().debug(f'Gripper state: {"open" if self.gripper_open else "closed"} (not implemented)')
                ...
                # Only apply control to the arm joints (indices 3-9), skipping base joints (0-2)
                # This effectively keeps the base fixed
                arm_actuator_ids = self.actuator_ids[3:]  # Skip base joints
                arm_dof_ids = self.dof_ids[3:]  # Skip base joints
                
                # Set control for arm joints only
                self.data.ctrl[arm_actuator_ids] = self.configuration.q[arm_dof_ids]
